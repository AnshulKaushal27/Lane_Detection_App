...
# ========================================
# 2. LOAD MODEL (Cached for efficiency)
# ========================================
@st.cache_resource
def load_model():
    ...
    if not os.path.exists(model_path):
        ...
        st.error("Please ensure 'best_lane_model_stage2.h5' is in the same directory as app.py")
        st.stop()
    
    try:
        with st.spinner("Loading lane detection model..."):
...
def process_video(video_path, model, progress_bar, status_text, alpha):
    ...
    try:
        ...
        while True:
            ...
            # Apply green overlay with user-selected alpha
...
def main():
    # Header
    st.markdown("# 🛣️ Lane Detection with AI")
    st.markdown("---")
